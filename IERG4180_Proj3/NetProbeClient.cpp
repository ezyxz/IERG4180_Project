...
int udpSend() {
    ...
    if (pktrate > 0) {
        while (1)
        {
            ...
            // printf("time_gap %f\n", time_gap);
            if (time_gap >= (double)mystat/1000) {
                double throughput = ((double)bytes_send_per_stat * 8) / (time_gap * 1000000);
    ...
    while (!if_error && (p_num_index < pktnum || pktnum == 0)) {
        ...
        if (total_send < single_iter_pkt_threshold || pktrate == 0) {
            ...
            while (bytes_sent < pktsize) {
                int r = send(sock_default, msg + bytes_sent, pktsize - bytes_sent, 0);
                if (r > 0) {
                    bytes_sent += r;
                }
                else {
...
int udpRecv() {
    ...
    LocalAddr.sin_port = htons(portPkt.port);
    ...
    char* client_ip = inet_ntoa(LocalAddr.sin_addr);
    int client_port = ntohs(LocalAddr.sin_port);
    ...
    if (strcmp(proto, "udp") == 0 || strcmp(proto, "UDP") == 0) {
        ...
        if (MODE == 0) {
            ...
            while (byte_recv < msg_size) {
                int r = recv(sock_default, conf_string, msg_size - byte_recv, 0);
                if (r > 0) {
                    byte_recv += r;
                }
                else {
            ...
            server_udp_addr.sin_port = htons(doki.port);
